...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout implements IPullToRefresh<T> {
    ...
    public final boolean isRefreshing() {
        return mState == State.REFRESHING || mState == State.MANUAL_REFRESHING || mState == State.REFRESH_DONE_WARN_ING;
    }

    @Override
    public final boolean isRefreshDoneWarning() {
        return mState == State.REFRESH_DONE_WARN_ING;
    }
    ...
    public final void onRefreshComplete() {
        if (isRefreshing() && !isRefreshDoneWarning()) {
            setState(State.REFRESH_DONE_WARN_ING);
        }
    }
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_UP: {
                if (mIsBeingDragged) {
                    ...
                    if (isLastPage && mCurrentMode == Mode.PULL_FROM_END) {
                        smoothScrollTo(0);
                        return true;
                    }
...
//		Log.e("PullToRefreshBase", "in setRefreshing(boolean doScroll)  and !isRefreshing()="+!isRefreshing());
        if (!isRefreshing()) {
    ...
    final void setState(State state, final boolean... params) {
        ...
        switch (mState) {
            case REFRESH_DONE_WARN_ING:
                onRefreshDoneWarning();
                break;
            case RESET:
                onReset();
                break;
...
//				Log.e("PullToRefreshBase", "onRefreshing(final boolean doScroll) mShowViewWhileRefreshing="+mShowViewWhileRefreshing);
                ...
                switch (mCurrentMode) {
                    ...
                    default:
                    case PULL_FROM_START:
//						Log.e("PullToRefreshBase", "PULL_FROM_START");
     ...
     */
    public void onReset() {
//		Log.e("PullToRefreshBase", "in onReset()  getScaleY()="+getScrollY());
        mState = State.RESET;
        mIsBeingDragged = false;
    ...
    boolean isAllReadyCallRefreshIng = false;

    public void stopRefreshing() {
//		Log.e("PullToRefreshBase", "------------------------------------------------------------in stopRefreshing()  getScaleY()="+getScrollY());
        mCurrentMode = Mode.PULL_FROM_START;
        isAllReadyCallRefreshIng = false;
        mState = State.RESET;
        mIsBeingDragged = false;
        mLayoutVisibilityChangesEnabled = true;
        mHeaderLayout.reset();
    ...
    protected void onRefreshDoneWarning() {
//	   Log.e("PullToRefreshBase", "in onRefreshDoneWarning()");
        mHeaderLayout.refreshDoneWarn();
        postDelayed(new Runnable() {
            @Override
            public void run() {
//				 Log.e("PullToRefreshBase", "in Runnable.onReset()");
                if (isAllReadyCallRefreshIng)
                    onReset();
            }
        }, 1400);
    }
    ...
    protected final void immediatelyScrollTo(int scrollValue) {
        smoothScrollTo(scrollValue, 0);
    }
...
//		Log.e("PullToRefreshBase", "callRefreshListener()  listView开始刷新数据");
...
//		Log.e("PullToRefreshBase", "smoothScrollTo() oldScrollValue != newScrollValue="+(oldScrollValue != newScrollValue));
    ...
    public void setLastPage(boolean isLastPage) {
        ...
        if (null != mFooterLayout) {
            mFooterLayout.setLastPage(isLastPage);
        }
    }
}
